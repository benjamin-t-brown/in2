		...
		this.typeFuncs = {
				...
				if (children.length === 0) {
					...
					if (ret.slice(0, 5) === 'error') {
						this.error(
							file.name,
							node.id,
							'Text node content could not be evaluated. ' + ret.slice(5) + `\n CONTENT ${node.content}`,
						);
						return null;
					}
			...
			choice: (node, file) => {
				...
				for (const i in children) {
					...
					ret += `    if(${conditional})\n    {\n`;
					ret += `        choices.push_back("${text_child.content.replace(/"/g, '\\"')}");\n`;
					ret += `        _post_conditionals.push_back("${post_conditional}");\n`;
					ret += `        _talk_indices.push_back(&${_get_function_id(text_child.id, file)});\n`;
					ret += `    }\n`;
				}
			...
			switch: (node, file) => {
				...
				for (const i in children) {
					...
					const { conditional, post_conditional } = _parse_conditional((child && child.content) || '', child, file);
					let args = null;
			...
			pass_fail: (node, file) => {
				...
				const { conditional, post_conditional } = _parse_conditional((node && node.content) || '', node, file);
				let ret =
					`// ${node.type}\n` +
					`void ${_get_function_id(node.id, file)}(Character* self)\n{\n` +
					//`    player.set( 'current_in2_node', '${node.id}' );\n` +
				...
				if (children.length === 0) {
					...
					if (ret.slice(0, 5) === 'error') {
						this.error(
							file.name,
							node.id,
							'Action node content could not be evaluated. ' + ret.slice(5) + `\n CONTENT ${node.content}`,
						);
						return null;
					}
					return ret + '\n' + this.compileNode(child, file);
				...
				if (children.length === 0) {
					...
					const node_list = content_list.map((content, i) => {
						...
						if (content[0] === '#' || content.length === 0) {
							local_node = _create_action_node(content.slice(1), id, { id: child_id }, file);
						} else {
					...
					const last_ret =
						`// ${node.type} LAST\n` +
						`void ${_get_function_id(node.id + '_' + node_list.length, file)}(Character* self)\n{\n` +
	...
	input_files.forEach((filename, i) => {
		...
		c.readAndCompile(filename, (result, file) => {
			...
			if (num_started === num_finished) {
				...
				aggregated_result += '\n';
				function_calls.forEach(({ id, file }) => {
					aggregated_result += `    ${_get_character_id(file)}->add_dialog(&${_get_function_id(
						id,
						file,
					)},"${_get_function_id(id, file)}");\n`;
				});
...
if (argv.file) {
	...
	compile([__dirname + '/../tree-builder/save/' + argv.file], '/out/' + argv.file + '.compiled.cpp');
} else if (argv.files) {
