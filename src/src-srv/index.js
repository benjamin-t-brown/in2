...
http_server.post('compile', (obj, resp, data) => {
	var cmd = `cd ${__dirname}/../../src/ && node compiler.js --files ` + data.files.join(',');
	console.log(cmd);
	exec(
		cmd,
		on_exec_compiled.bind(null, resp, (err, ret) => {
			if (ret.success) {
				ret.file = fs.readFileSync(`${__dirname}/../../src/main.compiled.js`).toString();
			}
...
http_server.get('compile', (obj, resp) => {
	var cmd = `cd ${__dirname}/../../src/ && node ${compiler}`;
	if (obj.event_args[0]) {
		...
		exec(
			...
			on_exec_compiled.bind(null, resp, (err, ret) => {
				if (ret.success) {
					ret.file = fs
						.readFileSync(`${__dirname}/../../src/out/${obj.event_args[0]}.compiled.${extension}`)
						.toString();
				}
				http_server.reply(resp, {
					err: err,
		...
		exec(
			...
			on_exec_compiled.bind(null, resp, (err, ret) => {
				if (ret.success) {
					ret.file = fs.readFileSync(`${__dirname}/../../src/main.compiled.${extension}`).toString();
				}
				http_server.reply(resp, {
					err: err,
					data: ret,
...
http_server.get('images', (obj, resp) => {
	fs.readdir(__dirname + '/../assets/img/', (err, dirs) => {
		var ret = {
			err: err,
			data: null,
		};
